...
#If a n attribute has a $ in front, it can be accessed as below.
#numeric attribute
...
?table
table(AdultUCI$education, useNA = 'always') 
...
age_p
# The curved brace means included and square bracket means not included.
#ladding labels
age_p = cut(age, c(0,15,25,67,200), labels = c("child", "pupil", "working", "pensioner"))
age_p
#This returns an error because the values are not ordered.
age_p[1] > age_p[3]
...
#View the dataset in R studio GUI.
View(AdultUCI)
...
#cross table
?xtabs
#The + sign is not an arithmetic sign but an intersection.
...
dim(AdultUCI)
#Apply deloneValued function to remove one valued attributes.
...
#Which attribute are single valued.
which(sapply(AdultUCI, function(x) {length(unique(x))}) == 1);
#Delete one valued columns.
AdultUCI <- delOneValued(AdultUCI)
colnames(AdultUCI)
